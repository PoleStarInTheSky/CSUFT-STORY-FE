...
export default function Register() {
  ...
  const registerWithParam = useCallback(() => {
    return register({ account, password })
  }, [account, password, register])
  const { execute, status, error } = useAsync(registerWithParam, false)
  //submit 事件的处理函数
  ...
  const accountFn: Strategy = useCallback((field) => {
    const accountRegexp = /^[0-9a-zA-Z]\w{3,19}$/
    if (accountRegexp.test(field)) {
      return { state: true }
      ...
      return {
        state: false,
        errorMsg: '账号应以数字或大小写字母开始，且无非法字符',
      }
    }
  }, [])
